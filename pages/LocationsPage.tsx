...
import { MapPin, Plus, Search, MoreHorizontal, Globe, Building, ChevronRight, Download } from "lucide-react";
...
export default function LocationsPage() {
  ...
  const handleExportCSV = () => {
    ...
    const columns = ["id", "code", "name", "group", "city", "country", "type", "status"];

    const header = columns.join(",");
    const rows = filtered.map((loc) => {
      return columns
        .map((col) => {
          const value = loc[col as keyof typeof loc] ?? "";

...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      try {
        ...
        // Fallback to mock authentication for demo/preview mode
  ...
  // Mock login for demo/preview mode when backend is unavailable
  const mockLogin = useCallback((email: string, password: string) => {
    console.log("[Auth] Using mock authentication for demo mode");
    ...
    if (demoUser && demoUser.password === password) {
      ...
      setToken(mockToken);
      if (typeof localStorage !== "undefined") {
        localStorage.setItem("nido_auth_token", mockToken);
      }
      setUser(demoUser.user);
      setUsers(Object.values(demoUsers).map((d) => d.user));
      console.log("[Auth] Mock login successful for", email);
      return true;
    }

    console.warn("[Auth] Mock login failed - invalid credentials");
    return false;
  }, []);

...
export default function AddClient() {
  ...
  const canProceed = () => {
    ...
    if (step === 1) {
      // Contract dates are optional for proceeding
      return (
        form.contractType &&
        form.pricingTier &&
        form.paymentTerms
  ...
  const save = async () => {
    console.log("[v0] save() called with form:", { companyName: form.companyName, email: form.email, primaryContactName: form.primaryContactName, gst: form.gst });
    if (!form.companyName?.trim() || !form.email?.trim()) {
      toast({ title: "Company name and email are required" });
      return;
    }
    if (!isValidEmail(form.email)) {
      console.log("[v0] email validation failed for:", form.email);
      toast({ title: "Please enter a valid email address" });
      return;
    }
    if (!form.primaryContactName?.trim()) {
      toast({ title: "Primary contact name is required" });
      return;
    }
    // Phone validation only if phone is provided
    if (form.contactNumber?.trim() && !isValidPhoneNumber(form.contactNumber)) {
    ...
    const clientData = {
      client_code: form.clientCode || nextSequentialCode(
        clientCodePrefix,
        clients.map((entry) => entry.clientCode),
        5,
      ),
    ...
    console.log("[v0] Saving to Supabase:", clientData);
    ...
    if (error) {
      ...
      toast({ title: "Failed to save client", description: error.message });
      return;
    }

    console.log("[v0] Client saved successfully:", data);
        ...
        <div className="max-w-4xl mx-auto rounded-2xl border bg-card p-6 shadow-sm space-y-6">
          ...
          <div className="flex gap-3 pt-4">
            ...
            {step < STEPS.length - 1 && (
              <Button
                onClick={() => {
                  console.log("[v0] Next clicked, step:", step, "canProceed:", canProceed());
                  if (canProceed()) {
                    setStep(step + 1);
                  }
                }}
            ...
            {/* Quick Save on Step 0 - allows skipping other steps for demo */}
            {step === 0 && canProceed() && (
              <Button
                onClick={save}
                variant="outline"
              ...
              >
                <Check className="h-4 w-4 mr-2" /> Quick Save
              </Button>
            )}
            {step === STEPS.length - 1 && (
              <Button

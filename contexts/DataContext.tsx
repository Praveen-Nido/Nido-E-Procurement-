...
export interface MasterCatalogItem {
  ...
  status: "published" | "draft" | "archived" | "In Stock" | "Low Stock" | "Out of Stock";
  image?: string;
  description?: string;
  initialStock: number;
...
useEffect(() => {
  ...
  const mapProductToCatalogItem = (
    ...
    // Preserve publication status if it exists
    let status: any;
    if (["draft", "published", "archived"].includes(rawStatus)) {
      status = rawStatus;
    } else {
      // Fall back to inventory-based status for backward compatibility
      status = quantity <= 0
        ? "Out of Stock"
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (vendorsResult.status === "fulfilled") {
          ...
          console.error("❌ VENDORS FETCH ERROR:", vendorsResult.reason);
          errors.push(`vendors: ${vendorsResult.reason?.message || "failed"}`);
          
          // Fallback: Load vendors directly from Supabase
          try {
            ...
            if (supabaseError) {
              console.log("[v0] Supabase vendor fetch also failed, will use localStorage", supabaseError);
            } else if (supabaseVendors && supabaseVendors.length > 0) {
              const mapped = supabaseVendors.map((v: any) => ({
                id: v.id,
                name: v.company_name,
              ...
              setVendors(mapped);
              console.log("[v0] Loaded vendors from Supabase fallback:", mapped.length);
            } else {
              console.log("[v0] No vendors in Supabase, will use localStorage vendors");
            }
          } catch (supabaseErr) {
            console.log("[v0] Supabase fallback error:", supabaseErr);
          }
        }
        ...
        // Only show errors if there are critical failures (not just API 500s)
        if (errors.length > 0) {
          // Log errors for debugging but don't show them as notifications
          // since the app works fine with local data when backend is unavailable
          console.log("[v0] Data refresh warnings:", errors);
          // Don't set error state for API failures - app continues with local data
        }
      } catch (error) {
    ...
    void fetchData().catch((err) => {
      console.error("[v0] Unhandled error in data context initialization:", err);
    });
  }, [
    isAuthenticated,
  ...
  const addClient = useCallback(
    (client: Partial<Client>) => {
      ...
      // Create a local client object for fallback when API is unavailable
      const localClient: Client = {
        id: `client-${Date.now()}`,
        clientCode,
        name: client.name || client.companyName || "Unnamed Client",
        companyName: client.companyName || client.name || "Unnamed Client",
        ...
        locationDetails: {
          address: client.locationDetails?.address || client.address || "",
        ...
        contractType: client.contractType || "Annual",
        pricingTier: client.pricingTier || "mid",
        paymentTerms: client.paymentTerms || "NET 30",
        companyLogo: client.companyLogo || "",
        contractDocuments: client.contractDocuments || [],
        notes: client.notes || "",
      ...
      void (async () => {
        try {
          ...
          // Fallback: Add client locally when API is unavailable (demo/preview mode)
          console.log("API unavailable, adding client locally:", localClient.clientCode);
  ...
  const addMasterCatalogItem = useCallback(
    async (item: Partial<MasterCatalogItem>) => {
      ...
      try {
        ...
        // If API fails, keep the optimistic item locally
        console.log("[v0] API failed, keeping optimistic item", apiError);
        const realItem: MasterCatalogItem = {
          ...optimisticItem,
  ...
  const updateMasterCatalogItem = useCallback(
    async (id: string, data: Partial<MasterCatalogItem>) => {
      ...
      await apiRequest<any>(`/api/products/${id}`, {
        ...
        // If API fails, keep the optimistic update locally
        console.log("[v0] API failed on update, keeping local changes", error);
      });

      await refreshMasterCatalogItems().catch(() => {
        // If refresh fails, data is already updated locally
        console.log("[v0] Refresh failed but local data is updated");
      });
      return true;
    },
  ...
  const deleteMasterCatalogItem = useCallback(
    async (id: string) => {
      try {
        ...
        console.log("[v0] Delete API failed, removing locally", error);
        // Remove locally even if API fails
        setMasterCatalogItems((prev) => prev.filter((item) => item.id !== id));
        return true;
      }
      await refreshMasterCatalogItems().catch(() => {
        console.log("[v0] Refresh failed after delete");
      });
      return true;
    },
    [refreshMasterCatalogItems],
  ...
  const refreshVendors = useCallback(async () => {
    try {
      ...
      if (error) {
        console.error("[v0] Failed to refresh vendors:", error);
        return;
      }
      ...
      if (supabaseVendors && supabaseVendors.length > 0) {
        ...
        setVendors(mapped);
        console.log("[v0] Vendors refreshed:", mapped.length);
      }
    } catch (err) {
      console.error("[v0] Error refreshing vendors:", err);
    }
  }, []);
